...
In this module, you will explore how assimilating data with different amounts of observation uncertainty and at different temporal frequencies affects forecasts of water quality at a lake that is monitored through the U.S. National Ecological Observatory Network (NEON).
...
**Objective 8.** Fit a model, calculate uncertainty, and generate a forecast for a different water quality variable.  
**Objective 9.** Determine the optimal frequency of data assimilation for forecasts of your water quality variable.

Example code is provided for Objectives 1-7, and you will be asked several short answer questions to interpret code output. Many of these short answer questions parallel (and in some cases are identical to) questions in the R Shiny app version of the module. Questions which are identical to those in the Shiny app will be indicated with **(Shiny)**, while questions unique to this RMarkdown will be indicated with **(Rmd)**. Note that question numbers will differ between the RMarkdown and the Shiny app, even if the question text is the same. Beginning in Objective 8, you will be guided to build on the module code by adjusting example code or developing your own code. Keep in mind that the example code is provided to help you, and use as much of it as you can in completing the questions embedded in Objective 8-9. There are a total of 59 questions. Please see the module rubric for possible points per question and confirm with your instructor whether and how the module will be graded.  
...
*Notice* that we are using a custom plotting function to plot these data. This is the first of several custom plots embedded throughout the module. If you would like to see the source code for these functions, it can be found in the `R` folder: `EnKF_and_plot_functions.R`.
```{r}
...
where $Chla$ is our timeseries of chlorophyll-a data, $\beta_0$ is the intercept parameter, $\beta_1$ is the coefficient on the 1-day lag of chlorophyll-a, and $\overline{Chla}$ is the mean of the chlorophyll-a timeseries, which we include to remove the effect of the mean concentration of chlorophyll-a over time from the autocorrelation between one day and the next. 
...
$$Chla_{t+1} = \beta_0 + \beta_1 * (Chla_{t} - \overline{Chla}) + \overline{Chla}$$
...
```{r}
plot_ic_dist(curr_chla, ic_distribution, x.lab = "Chlorophyll-a (ug/L)")
```
...
$$Chla_{t+1} = \beta_0 + \beta_1 * (Chla_{t} - \overline{Chla}) + \overline{Chla} + W$$
...
```{r}
plot_process_dist(process_distribution, x.lab = "Chlorophyll-a (ug/L)")
```
...
```{r}
plot_fc_dist(forecast_chla, x.lab = "Chlorophyll-a (ug/L)")
```
...
*Hint 1: Be sure to check the y axes when you are comparing the figures, as they are not identical.*

*Hint 2: Remember, even though there is only one white distribution, there are two forecasts plotted here! The forecast for 2020-09-26 is first generated, then subsequently used as the initial condition to forecast 2020-09-27, so the blue initial condition distribution is plotted directly over top of the white forecast distribution for 2020-09-26.* 
...
plot_ic_dist(curr_chla, ic_distribution_low, x.lab = "Chlorophyll-a (ug/L)")
```
...
plot_ic_dist(curr_chla, ic_distribution_high, x.lab = "Chlorophyll-a (ug/L)")
```
...
```{r}
plot_many_forecasts(forecast_data = forecast_data_no_da, forecast_series = forecast_series_no_da, forecast_var = "chla", y.lab = "Chlorophyll-a (ug/L)")
```
...
```{r}
plot_many_forecasts_with_obs(forecast_data = forecast_data_no_da, forecast_series = forecast_series_no_da, observations = chla_observations, forecast_var = "chla", y.lab = "Chlorophyll-a (ug/L)")
```
...
```{r}
plot_many_forecasts(forecast_data = forecast_data_weekly, forecast_series = forecast_series_weekly, forecast_var = "chla", y.lab = "Chlorophyll-a (ug/L)")
```
...
Then, we will plot the forecasts again, this time with the observations that occurred during the forecast period.
```{r}
plot_many_forecasts_with_obs(forecast_data = forecast_data_weekly, forecast_series = forecast_series_weekly, observations = chla_observations, forecast_var = "chla", y.lab = "Chlorophyll-a (ug/L)")
```
...
Let's plot our series of 1-day-ahead forecasts with weekly data assimilation.
```{r}
plot_many_forecasts(forecast_data = forecast_data_daily, forecast_series = forecast_series_daily, forecast_var = "chla", y.lab = "Chlorophyll-a (ug/L)")
```
...
Then, we will plot the forecasts again, this time with the observations that occurred during the forecast period.
```{r}
plot_many_forecasts_with_obs(forecast_data = forecast_data_daily, forecast_series = forecast_series_daily, observations = chla_observations, forecast_var = "chla", y.lab = "Chlorophyll-a (ug/L)")
```
...
```{r}
lake_data <- read_csv("./data/BARC_surfN_micromolesPerLiter.csv", show_col_types = FALSE) %>%
  rename(var = V1) %>% 
  filter(year(datetime) >= 2019) # %>%
  # mutate(var = ifelse(var < 0, 0, var)) # DETERMINE WHETHER YOU SHOULD TRUNCATE YOUR VARIABLE AT 0; IF SO, UNCOMMENT THIS LINE AND THE PIPE ON THE LINE ABOVE
...
```{r}
ggplot(data = lake_data, aes(x = datetime, y = var))+ #edit the y variable name
    geom_point(aes(color = "var"))+ # REPLACE 'var' WITH THE CORRECT NAME OF YOUR VARIABLE
    xlab("")+
    ylab(expression(paste("VARIABLE NAME")))+ # EDIT THE AXIS TITLE AND UNITS; water temperature will be degrees Celsius; dissolved oxygen is mg/L; surface nitrogen is micromoles per liter
    scale_color_manual(values = c("var" = "orange"), name = "")+ # REPLACE 'var' WITH THE CORRECT NAME OF YOUR VARIABLE; be sure it matches the name you provide to the geom_line() function above
    theme_bw()
```

Fit an autoregressive model to your data. 

```{r}
forecast_start_date <- "2020-08-02"

autocorrelation_data <- lake_data %>%
    filter(datetime < forecast_start_date) %>%
    mutate(var = na.approx(var, na.rm = F)) %>% 
...
ar_model <- ar.ols(model_data$var, order.max = 1, aic = FALSE, 
                     intercept = TRUE, demean = TRUE)
```
...
```{r}
mod <- intercept + ar1 * (model_data$var - var_mean) + var_mean
...
**Q.46 (Rmd) Now, we can assess our model visually. We will plot the model predictions and observations. Alter the code to plot your water quality variable.**
...
**Q.47 (Rmd) Assess your model fit to data by visually inspecting the plot above. How good is your model fit?**
...
```{r}
rmse <- round(sqrt(mean((mod - model_data$var)^2, na.rm = TRUE)), 2) 
rmse

bias <- mean(mod - model_data$var, na.rm = TRUE) 
bias
```

**Q.48 (Rmd) Assess your model fit to data using the values of bias and RMSE. How good is your model fit?**

**Answer Q.48**


Save residuals to calculate process uncertainty later.

```{r}
residuals <- mod - model_data$var 
```

Calculate initial conditions and process uncertainty.

Set the number of ensemble members (you can edit this number if you like).
```{r}
n_members = 500
```

**Q.49 (Rmd) Alter the code to read in high-frequency data of your chosen water quality variable. Choose from:**

1. water temperature: wtemp_celsius_highFrequency.csv.   
2. dissolved oxygen: DO_milligramsPerLiter_highFrequency.csv.   
3. surface nitrogen: surfN_micromolesPerLiter_highFrequency.csv.

**Answer Q.49** Edit the code block to provide your answer.
```{r}
high_frequency_data <- read_csv("./data/surfN_micromolesPerLiter_highFrequency.csv", show_col_types = FALSE) %>%
  rename(var = nitrate) # EDIT THIS SO THAT YOUR VARIABLE IS RENAMED TO 'var'; for water temperature, the original column name will be 'wtemp'; for dissolved oxygen it will be 'DO'; for nitrogen it will be 'nitrate'
```

**Q.50 (Rmd) Alter the code below to plot high-frequency data of your chosen variable.**

**Answer Q.50** Edit the code block to provide your answer.

```{r}
ggplot(data = high_frequency_data)+
  geom_line(aes(x = time, y = var, group = day, color = as.factor(day)))+ 
  theme_bw()+
  labs(color = "Day of experiment")+
  xlab("Hour of day")+
  ylab("VARIABLE NAME") # EDIT THE AXIS TITLE HERE
```

Calculate the standard deviation of the variable each day.

```{r}
ic_sd_dataframe <- high_frequency_data %>%
  group_by(day) %>%
  summarize(daily_sd_var = sd(var, na.rm = TRUE)) 
  
ic_sd <- mean(ic_sd_dataframe$daily_sd_var, na.rm = TRUE) 
ic_sd
```

Generate an initial condition distribution of your chosen variable.
...
**Q.51 (Rmd) Alter the code below to plot the initial condition distribution of your chosen variable.**

**Answer Q.51** Edit the code block to provide your answer.

```{r}
plot_ic_dist(curr_value, ic_distribution, x.lab = "YOUR VARIABLE NAME HERE") # EDIT THE VARIABLE NAME
```
...
**Q.52 (Rmd) Alter the code below to plot the process uncertainty distribution of your chosen variable.**

**Answer Q.52** Edit the code block to provide your answer.

```{r}
plot_process_dist(process_distribution, x.lab = "YOUR VARIABLE NAME HERE") # EDIT THE VARIABLE NAME
```
...
Create a `forecast_data` data frame, which is the data that will be provided to the model for forecasting.
...
forecast_data <- lake_data %>%
  ...
  mutate(datetime = as.Date(datetime)) %>%
  filter(datetime %in% forecast_dates) %>%
  mutate(var = ifelse(datetime %in% variable_assimilation_dates,var,NA)) 
```

Create a data frame (`forecast_series`) to hold the initial conditions and forecasts for each day in our forecast period.
...
Create a temporary data frame for the initial conditions distribution that matches the format of `forecast_series`.
...
Run a series of forecasts for your chosen water quality variable.
...
for(i in 2:length(forecast_dates)){
  ...
  #Select current row of forecast_data to see if there is data to use for updating
  new_obs <- forecast_data$var[i] #edit this to be your chosen variable name!
  ...
  #Build temporary data frame to hold current initial condition and forecast
  temp <- tibble(date = rep(forecast_dates[i], times = n_members*2),
               var = c(forecast_variable, ic_update), 
               ensemble_member = rep(1:n_members, times = 2),
               data_type = rep(c("fc","ic"), each = n_members))
...
**Q.53 (Rmd) Alter the code below to plot the series of one-day-ahead forecasts for your chosen variable.**

**Answer Q.53** Edit the code block to provide your answer.

```{r}
plot_many_forecasts(forecast_data = forecast_data, forecast_series = forecast_series, forecast_var = "var", y.lab = "YOUR VARIABLE NAME HERE") # EDIT THE y.lab ARGUMENT
```

**Q.54 (Rmd) Describe how the mean value of the 1-day-ahead forecasts changes over time when no data are assimilated.** 

**Answer Q.54**


**Q.55 (Rmd) Describe how the uncertainty distribution of the 1-day-ahead forecasts changes over time when no data are assimilated.**
...
forecast_means <- forecast_series %>%
  filter(data_type == "fc") %>%
  group_by(date) %>%
  summarize(forecast_mean = mean(var, na.rm = TRUE)) 
```
...
**Q.56 (Rmd) Alter the code below to plot the series of one-day-ahead forecasts for your chosen variable with observations.**

**Answer Q.56** Edit the code block to provide your answer.

```{r}
plot_many_forecasts_with_obs(forecast_data = forecast_data, forecast_series = forecast_series, observations = variable_observations, forecast_var = "var", y.lab = "YOUR VARIABLE NAME HERE") #EDIT THE y.lab ARGUMENT
```

**Q.57 (Rmd) Using the plot above, visually assess the forecasts and describe their accuracy. How well do they match observations?** 

**Answer Q.57**


Calculate the bias and RMSE of your forecasts.

```{r}
bias <- mean(forecast_means$forecast_mean - variable_observations$var, na.rm = TRUE) 
bias

rmse <- round(sqrt(mean((forecast_means$forecast_mean - variable_observations$var)^2, na.rm = TRUE)), 2) 
rmse
```

**Q.58 (Rmd) Use the values of bias and RMSE to assess the forecasts with no data assimilation. How well do you think the forecasts are performing?**
...
**Q.59 (Rmd) What is the optimal frequency of data assimilation (in days) for your chosen water quality variable? What is the RMSE for the optimal frequency of data assimilation?**
