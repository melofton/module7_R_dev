...
```{r}
pacf_list <- acf(autocorrelation_data$chla, type = c("partial"), plot = TRUE)
...
Today, we will fit a simple form of an autoregressive, or AR model, which uses yesterday's chlorophyll-a observation to predict today's observation. This model can be written as:
...
Now, we will use some high-frequency (5-minute) chlorophyll-a data from our lake to estimate initial conditions uncertainty. First, we can look at variability in this 5-minute data over the course of a day to get a visual understanding of the daily variability in chlorophyll-a.
```{r}
#GOT stuck here - need plot that groups by day and plots time of day on x axis
...
ggplot(data = high_frequency_data)+
  geom_line(aes(x = hms, y = chla, group = date))+
  theme_bw()
```
