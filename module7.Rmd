...
A deterministic forecast is one that does not account for any uncertainty associated with predictions.
...
To generate a forecast for many days into the future using our model, we will start by using today's observation to make a prediction for tomorrow, and then use the prediction for tomorrow to predict two days from now, and then use the prediction for two days from now to predict three days from now, and so on.
...
## 5. Generate a forecast with **initial conditions** uncertainty. 
