...
plot_chla = function(est_out, lake_data, obs_file, start, stop, n_en){
  ...
  plot_data <- tibble(datetime = rep(plot_dates, each = n_en),
    ...
    mutate(chla = exp(chla),
           ...
           chla_ic_ens = ifelse(assim_key1 == 1, chla_ic, NA),
           chla_connect_ens = ifelse(assim_key2 == 1, chla_pred, NA),
           chla_pred_ens = ifelse(assim_key3 == 1, NA, chla_pred)) 
    
  p <- ggplot(data = plot_data, aes(x = datefactor))+
    geom_line(aes(y = chla_pred_ens, group = ensemble_member, color = "Ensemble members"))+
        geom_line(aes(y = chla_ic_ens, group = ensemble_member, color = "Ensemble members"))+
    geom_line(aes(y = chla_connect_ens, group = ensemble_member, color = "Ensemble members"))+
    geom_violin(aes(y = chla_fc, fill = "Forecast for today"), color = "black",
                scale = "width", width = 0.7)+
    geom_violin(aes(y = chla_ic, fill = "Initial condition for tomorrow's forecast"), color = "cornflowerblue", alpha = 0.4, scale = "width", width = 0.7)+
    geom_point(aes(y = chla, color = "Non-assimilated observations"))+
    geom_point(aes(y = obs_assimilated, color = "Assimilated observations"))+
...
est_out = EnKF(n_en = n_en, 
           ...
           init_cond_cv = c(0.025),#cv for chl-a 
           state_names = c("chla"),
           yini = yini)
